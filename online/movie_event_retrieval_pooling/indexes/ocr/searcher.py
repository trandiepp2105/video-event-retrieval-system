...
class OCRFuzzyScorer:
    ...
    @staticmethod
    def _ngrams_ratio(q_split: list[str], d_split: list[str]) -> float:
        if not q_split:
            return 0.0
...
class OCRSearcher:
    ...
    def search(self, query: str, top_k: int) -> list[OCRSearchHit]:
        ...
        for item in payload.get("hits", []):
            ...
            text = str(item.get("text_raw") or item.get("text_clean") or "")
            score = self.scorer.score(query_norm, text)
            if score > 0:
                rescored.append((score, ocr_id, text))
        rescored.sort(key=lambda value: value[0], reverse=True)
        hits: list[OCRSearchHit] = []
        for rank, (score, ocr_id, text) in enumerate(rescored[: int(top_k)], start=1):
            hits.append(
                OCRSearchHit(
                    ocr_id=ocr_id,
                    score=float(score),
