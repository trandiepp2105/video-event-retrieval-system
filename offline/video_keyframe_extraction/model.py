...
class CLIPEmbedder:
    ...
    def _load_model(self):
        ...
        pretrained_value = str(self.config.clip_pretrained)
        if not Path(pretrained_value).exists():
            raise FileNotFoundError(
                f"Khong tim thay pretrained weights local: {pretrained_value}. "
                "Module nay duoc thiet ke de chay offline."
            )
